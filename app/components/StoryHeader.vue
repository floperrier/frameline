...
const { id, story, keptAt, writing, change, write } = defineProps<{
   ...
   * is stays, because it is what the Author is inside; where it can be read
   * folds to the one act that puts it there, because the Synopsis and the public
   * link are three hundred pixels of a screen whose subject is a Shot.
   */
  writing?: boolean
...
/* Writing a Scene is a state of the bench and the header takes it too, the way
   the graph beside it folds into a rail: what the Story is stays — an Author
   inside a Scene is still inside that Story — and where it can be read folds to
   the one act that puts it there, on the same line. The title comes down to a
   label on a reel, because the three columns below it are what the screen is
   for. Nothing moves out of the tab order and nothing changes shape: the two
   halves are the two halves either way. */
header.writing {
