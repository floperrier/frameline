...
/**
 * Draws an Exit from one Scene to another. Both ends are looked up in the one
 * statement that writes the Exit, joined on their Story: an Exit cannot leave the
...
export default defineEventHandler(async (event) => {
  ...
  const { rows } = await useDb().execute<Exit>(sql`
    ...
    select
      departure.id,
      arrival.id,
      coalesce((select max(position) + 1 from exits where from_scene_id = departure.id), 0)
